...
transformed parameters {
  ...
  matrix[M, R] beta2;
  {
    ...
    for (r in 1:R)
      lambda_tilde[,r] = sqrt( c2 * square(lambda[,r]) ./ (c2 + square(tau[r]) * square(lambda[,r])) );
    for (r in 1:R)
      lambda_tilde2[,r] = sqrt( c2 * square(lambda2[,r]) ./ (c2 + square(tau2[r]) * square(lambda2[,r])) );
    // beta ~ normal(0, tau * lambda_tilde)
    for (r in 1:R) beta[,r] = tau[r] * lambda_tilde[,r] .* beta_tilde[,r];
    // beta ~ normal(0, tau * lambda_tilde)
    for (r in 1:R) beta2[,r] = tau2[r] * lambda_tilde2[,r] .* beta_tilde2[,r];
  }
}
model {

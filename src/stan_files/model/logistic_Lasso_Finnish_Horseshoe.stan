...
//  int<lower=1> R; // Number of random effects
  matrix[N, M] X; // matrix of features
  matrix[N, L] Z; //matrix of covariates
 // matrix[R, N] S; //matrix of site
...
model {
 ...
 // sigma_zeta ~ normal(0, 2);
  ...
  y ~ bernoulli_logit(Z * eta + X * beta + alpha);
}
generated quantities {
  vector[N] yhat;
  for (n in 1:N) {
    yhat[n] = exp(Z[n] * eta + X[n] * beta + alpha)/(1+exp(Z[n] * eta + X[n] * beta + alpha));
  }
}
